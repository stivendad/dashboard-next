...
import { PropsItemSidebar } from './SidebarMenuItem';

export const Sidebar = () => {

    type MenuItems = PropsItemSidebar[] ;
    ...
    return (
        <div id="menu" className="bg-gray-900 min-h-screen z-10 text-slate-300 w-64 left-0 h-screen fixed overflow-y-scroll min-w-96">
            ...
            <div id="nav" className="w-full px-6">
                ...
                { menuItems.map( item => (
                    <SidebarMenuItem 
                        key={item.path}
                        {...item}
                        // todo: como el item ya tiene los elementos necesarios se puede aplicar de la forma anterior o 1 x 1
                        // path={item.path} 
                        // icon={item.icon} 
                        // title={item.title} 
